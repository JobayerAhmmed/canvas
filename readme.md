...
11. Create student feedback files
...
2. Course ID: Open the course by clicking on the course link.
...
3. Assignment ID: Click on the `Assignments` (or, whatever name is used 
...
Here, number 1111111 is the assignment id. Update file `files/assignment_id.txt` with this number.

4. Start and End Student IDs: Click on the SpeedGrader. In SpeedGrader, you find each student's submission. 
...
222222 is the student ID for John Doe. Update file `files/student_id_start.txt` with this number.
Do the same for Jane Smith and update file `files/student_id_end.txt`.

5. Get the student names and IDS, and save them in an excel file.
...
`students.xlsx` file contains student names in the order found in SpeedGrader.
...
1. Update file `files/excel_columns.txt` with the above column names. 
   Each line in the file should contain one column name. Do not change the
   order of the first five column names in the .txt file, from Student Name to
   Early Days, otherwise, the code will not work.

2. Update function `convert_utc_to_iowa()` in utils.py file. This function
converts UTC time to Iowa local time. Just change the hours argument in timedelta.

3. Run: `python3 create_excel.py`. It will create `grades.xlsx` file with the column names.
...
1. Run: `python3 download_submissions.py`

2. Open `grades.xlsx` and you will find Submitted At column is updated with submission time.
...
## Step 8: Prepare Submissions for Grading

In this step, you will extract all zip files, make a copy of the grader for each student,
and copy the Java files to the `hw1` directory inside the grader.
...
There will be a new directory named `submissions_with_grader` inside the `data/hw1` directory.
...
2. Early Points: `=IF(AND(E2>0,E2<=10),SUM(H2:I2)*(E2*0.01),0)`
...
2. Update `grades.xlsx` for each student.

## Step 11: Create Student Feedback Files
